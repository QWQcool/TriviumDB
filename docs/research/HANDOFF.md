...
1. **去均值修复推广到全部四档**（`wolt_clip` 71.48% vs 论文 70.68% ⇒ 校准 **7/12 行**；
   ③ 完成后为 **11/12 行**：+`minilm-384` 88.94/88.09、`bge-m3-1024` 94.91/93.81、
   `dbpedia-1536` 94.64/95.34、`dbpedia-3072` 95.70/95.65；**`redcaps-512` 源不可得**）：
...
| **U19** | ✅ **已完成：7/7 去均值臂全部从加权导航受益**（增益叠加）——GIST `4.38→82.77%`（≈19×）、SIFT `47.23→92.47`、GloVe `71.69→93.32`、cohere `97.51→98.91`；`sphere` 走完全流程仍 2.91%（**不可修复** ✓ 符合判据分工）。**竞争力判定再修正**：glove100 支配→**平价（≈1.07×）**、sift128 差距 50pp→**5pp**、gist960 仍被支配 | 见 `f1-metric-consistency.md` §4.1、`audit-and-direction.md` §9.5 |
...
| **U15** | ✅ **已完成（D2 + Phase 2 的 7 臂，R7 关闭）**：`wolt_clipc` **支配→曲线相交**（≥89% 区快 2.3–3.9×）、`glove100c` **支配→平价（≈1.07×）**、`gist960c`/`sift128c` 差距缩到 **2.1pp**/**4.6pp** 但仍被支配、`spherec` **HNSW 也崩 1.40%**（QuIVer 同召回快 6.3–7.9×）、`randomc` 快 **7.0–8.5×**、`coherec` 仍胜但收窄到 **2.7–3.4×**（centered 任务对 HNSW 也更容易）⇒ 完整表见 `audit-and-direction.md` §9.6 | 余下：**`gist960c` 的竞品曲线未测**（U18） |
| **U18** | ✅ **已完成**：`gist960c` 竞品 —— HNSW `ef=64` = **84.85%** @6,734 vs 流水线上限 **82.77%** @4,717 ⇒ **仍被支配**（差距 **2.1pp**）；`faiss_exact` = 99.93% ⇒ 无并列问题 | — |
| **U16** | ✅ **已完成（③）**：`minilm-384` 88.94（论文 88.09）、`bge-m3-1024` 94.91（93.81）、`dbpedia-1536` 94.64（95.34）、`dbpedia-3072` 95.70（95.65）⇒ **11/12 行复现，最大偏差 1.84pp**。**`redcaps-512` 判定为源不可得**：HF 上 `redcaps` 只有原始图文数据集，`Qdrant/` 名下无嵌入版 | 已完结 |
| **U20** | ✅ **已完成**：脚本默认协议改为 **K 个查询子集取均值**（`GATE_K`，默认 K=3；JSON 存 `probe_k`/`probe_raw`/`probe_topef_min_range`）。**26 臂按 K=3 重跑：全部裁决不变、probe 值变化 ≤2.0pp**（glove100 45.1→44.8、glove100c 45.6→44.9）⇒ **§2 表对协议稳健**；`glove100c` 的 Hamming 口径极差仍跨 50%（44.1–50.8%）⇒ 引用该行须同时给 K 与极差 | 已完结 |
