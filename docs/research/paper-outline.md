...
> **第三方评估 + 构造性修复**：我们独立复现了 QuIVer（PVLDB'27）的适用性主张（11/12 行），
...
| §4 Reproduction | **表 1：11/12 行校准**（含 Δ 与 `redcaps` 不可复现的三条证据）；4.6–5.0× 加速复核；`faiss_exact` 地板 | `audit-and-direction.md` §1 | ✅ 齐 |
...
| # | 缺口 | 成本 | 是否阻塞动笔 |
|---|---|---|---|
| **G-c** | **图保真度 `L0∩cos_top64` 补到 ≥6 个数据集**（现只有 gist960/gist960c/sift128/cohere） | 0.5 天（每个建图 + CSR 导出） | ❌ 不阻塞 §1–§5/§7/§8；只阻塞 §6 的完整表 |
| **G-e** | 第二台 **AVX-512** 机器复测倍差 | 外部依赖 | ❌ 不阻塞；写清为限制即可 |
| G-h | §9 的"决策表"与相关工作（RaBitQ/PQ/OPQ/All-but-the-Top） | 0.5 天 | ❌ |
| G-i | 关键点 ≥3 次重复 + 置信区间（目前用 G-det 声明代替） | 0.5 天 | ❌ |
| ~~G-a~~ | ~~剩余 5 行数据集~~ | — | ✅ 已完成（11/12） |
